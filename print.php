 ...
 *
 */
//function for calculating a checksum digit based off of USS.C39 mod 43
function checkerSum( $string )
{
	$checksum = 0;
...
for ($c=0; $c<$cols; $c++) {
    ...
    for ($r=0; $r<$rows; $r++) {
        ...
        // RR: this is the part I wrote, calls the checksum digit function on the barcode so far
        // and concatenates that digit with the original if checksum == true 
        $chky = checkerSum($txt);
        $barTxt= $txt;
        if ($checksum){
            $barTxt .= $chky;
        }
        $pdf->write1DBarcode( $barTxt, 'C39', $posX, $posY+$paddingTop, $wLabel, $hBarcode, '', $style);
    }
}
